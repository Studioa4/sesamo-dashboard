...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      ...
      console.error('Errore caricamento impianti:', JSON.stringify(err.response?.data || err.message, null, 2));
    ...
    try {
      ...
      if (err.response && err.response.data) {
        ...
        res.status(500).json({
          error: err.response.data.message || 'Errore creazione impianto',
